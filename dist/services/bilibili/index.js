...
const bili_api_1 = __importDefault(require("bili-api"));
...
function updateDynamic() {
    mids.forEach(async (val, key) => {
        const new_dynamic = [];
        const old_dynamic = await util_1.getProfile(val.toString(), './data/dynamic');
        logger.mark(`正在获取 bilibili ${key} 动态...`);
        bili_api_1.default({ mid: val }, ['dynamics'])
            .then(data => {
            ...
            for (const dynamic of dynamicsRaw) {
                ...
                switch (type) {
                    ...
                    case 4:
                        ...
                        msg += content;
                        for (const { img_src } of reprinted)
                            msg += `\n[CQ:image,file=${img_src}]`;
                        break;
                    case 64:
...
// 每 5 分钟监听动态
node_schedule_1.scheduleJob('0 0/5 * * * ?', async () => {
    updateDynamic();
});
